# git-surgeon — Command Reference

Full `git-surgeon` command catalog. **Under this skill's rules, everything
except `hunks`, `show`, and `stage` is FORBIDDEN.** The agent runs only
read-only inspection plus staging. The remaining commands are documented here
so the agent can suggest them to the user — not run them itself.

## Forbidden in this skill

| Command       | Why forbidden                                        |
| ------------- | ---------------------------------------------------- |
| `commit`      | Commits — only the user commits.                     |
| `commit-to`   | Commits to another branch.                           |
| `unstage`     | Mutates the index beyond staging.                    |
| `discard`     | Destructive — throws away working-tree changes.      |
| `fold`        | Rewrites history.                                    |
| `amend`       | Rewrites history.                                    |
| `reword`      | Rewrites history.                                    |
| `squash`      | Rewrites history.                                    |
| `undo`        | Reverse-applies commit hunks to the working tree.    |
| `undo-file`   | Reverse-applies commit file changes.                 |
| `split`       | Rewrites a commit into multiple.                     |
| `move`        | Reorders history.                                    |
| `update`      | Package upgrade, out of scope for dissection.        |

If a task seems to require any of these, STOP and ask the user.
...
## Typical workflow (reference; NOT this skill's workflow)

The vendor-intended general workflow is:
...
This skill overrides step 3/5: the agent only stages; the user commits.

## Committing to another branch (forbidden — for user reference)
...
## Picking the right folding command (forbidden — for user reference)
...
## Folding existing commits into earlier commits (forbidden — for user reference)
...
(In this skill, the agent uses `--blame` for grouping only and never runs
`commit` or `fold`; it may suggest the above to the user.)

## Amending earlier commits with staged changes (forbidden — for user reference)
...
## Squashing commits (forbidden — for user reference)
...
## Undoing changes from commits (forbidden — for user reference)
...
## Splitting commits (forbidden — for user reference)
...
## Moving commits (forbidden — for user reference)
