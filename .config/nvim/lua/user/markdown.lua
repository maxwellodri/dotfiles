...
vim.api.nvim_create_autocmd('FileType', {
    ...
    callback = function()
        ...
        vim.keymap.set('n', 'O', function()
            local below = vim.fn.line('.')
            local blank_lines = vim.fn['repeat']({ '' }, vim.v.count1)
            vim.api.nvim_buf_set_lines(0, below, below, false, blank_lines)
            vim.api.nvim_win_set_cursor(0, { below + vim.v.count1, 0 })
            vim.cmd('startinsert!')
        end, { buffer = true, desc = 'Empty line below' })
    end,
})
