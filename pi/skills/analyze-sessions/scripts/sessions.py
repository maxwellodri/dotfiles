...
A session is a JSONL file under the sessions root (default
~/.local/state/pi/sessions, override with PI_CODING_AGENT_SESSION_DIR),
named <ISO-timestamp>_<uuid>.jsonl. Nested subagent transcripts
(<encoded-cwd>--/<parent-id>/<subagent-id>/run-N/<file>.jsonl) are also
...
SESSIONS_ROOT = Path(os.path.expanduser(
    os.environ.get("PI_CODING_AGENT_SESSION_DIR", "~/.local/state/pi/sessions")
))
