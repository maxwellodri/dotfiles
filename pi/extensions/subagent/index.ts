...
/** Only turns, context size, and model are shown — token/cost figures are
 *  noise (subscription-backed; see the footer.ts stats note). */
function formatUsageStats(usage: { turns?: number; contextTokens?: number }, model?: string): string {
	const parts: string[] = [];
...
/**
 * Resolve how to re-invoke pi for a child subagent.
 *
 * CRITICAL: never use bare `pi` on PATH on this machine — it resolves to the
 * user's wrapper (~/bin/pi -> scripts/pi), which paints a gruvbox OSC canvas,
 * re-reads the API key from pass, and sets a session dir. All pointless for a
 * headless JSON-mode child, and the OSC sequences are noise in JSON output.
 *
 * The wrapper always launches the REAL binary directly as process.argv[1], so
 * the first branch (re-invoke node with that same script) is what fires in
 * practice here, fully bypassing the wrapper. The later fallbacks only exist
 * for unusual launches and are kept wrapper-safe.
 */
function getPiInvocation(args: string[]): { command: string; args: string[] } {
	...
	// If process.execPath is pi itself (compiled binary), re-invoke directly.
	if (!isGenericRuntime) {
		return { command: process.execPath, args };
	}

	// Generic node/bun runtime but no usable script in argv: resolve the real
	// npm-installed entrypoint rather than PATH `pi` (which is the wrapper).
	const realBin = path.join(os.homedir(), ".local", "bin", "pi");
	if (fs.existsSync(realBin)) {
		return { command: process.execPath, args: [realBin, ...args] };
	}

	// Nothing left to try. Surface the risk rather than silently hitting wrapper.
...
async function runSingleAgent(
	...
	if (!agent) {
		...
		return {
			...
			usage: { contextTokens: 0, turns: 0 },
			step,
		};
	}

	// Headless, ephemeral, fully pinned: JSON output, no session, no extensions.
	// Tools/model/system-prompt come from the agent definition (--tools/--model/
	// --append-system-prompt), so the child is deterministic and side-effect-free.
		...
		renderResult(result, { expanded }, theme, _context) {
			...
			const aggregateUsage = (results: SingleResult[]) => {
				...
				for (const r of results) {
					turns += r.usage.turns;
					contextTokens = Math.max(contextTokens, r.usage.contextTokens);
				}
